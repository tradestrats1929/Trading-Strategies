...
function tradeTypeOptions(instrument: InstrumentType): { value: TradeType; label: string }[] {
  if (isEquity(instrument)) return [
    { value: 'delivery', label: 'Delivery' },
    { value: 'intraday', label: 'Intraday' },
  ]
  if (isFutures(instrument)) return [
    { value: 'regular', label: 'Regular' },
  ]
  return [
    { value: 'regular', label: 'Regular square-off' },
    { value: 'exercise', label: 'Exercise / Expiry' },
  ]
}
...
const BREAKDOWN_ROWS: { key: keyof CostBreakdown; label: string; hint: string }[] = [
  { key: 'brokerage',   label: 'Brokerage',               hint: 'Zerodha: ₹0 delivery, ₹20 flat FNO, min(₹20, 0.03%) intraday' },
  ...
  { key: 'gst',         label: 'GST (18%)',               hint: 'On brokerage + exchange TC + SEBI fee (not on STT or stamp duty)' },
  { key: 'ipft',        label: 'IPFT',                    hint: 'Investor Protection Fund Trust — NSE only, ₹0.01/crore' },
  { key: 'dp_charges',  label: 'DP Charges',              hint: '₹15.34/scrip/day — equity delivery sell only (Zerodha)' },
]
...
export default function App() {
  ...
  const set = (field: keyof TradeForm) => (e: React.ChangeEvent<HTMLSelectElement | HTMLInputElement>) => {
    ...
    setForm(prev => {
      ...
      // Auto-reset trade_type when instrument changes
      if (field === 'instrument') {
        const opts = tradeTypeOptions(value as InstrumentType)
        next.trade_type = opts[0].value
      }
      return next
    })
  }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (isOptions(form.instrument)) {
      ...
      if (form.trade_type === 'exercise') {
        body.strike = parseFloat(form.strike)
        body.spot = parseFloat(form.spot)
      }
    }

    try {
      const r = await fetch(`${API_URL}/calculate`, {
        method: 'POST',
  ...
  return (
    <main style={{ fontFamily: 'sans-serif', padding: '2rem', maxWidth: '680px' }}>
      <h1 style={{ marginBottom: '0.25rem' }}>Transaction Cost Calculator</h1>
      <p style={{ color: 'grey', fontSize: '0.8rem', marginTop: 0 }}>
        Zerodha / NSE·BSE · Indian Equities & F&O · Rates: April 2026
      </p>
      ...
      <form onSubmit={handleSubmit}>
        <div style={grid}>
          ...
          <label style={lbl}>{isOptions(form.instrument) ? 'Premium (₹)' : 'Price (₹)'}
            <input type="number" min="0" step="0.05" required value={form.price}
              onChange={set('price')} style={inp} placeholder="e.g. 100" />
          </label>

          <label style={lbl}>Quantity (units)
            <input type="number" min="1" step="1" required value={form.quantity}
              onChange={set('quantity')} style={inp} placeholder="e.g. 50" />
          </label>
          ...
          {showExerciseFields && (
            <>
              <label style={lbl}>Strike Price (₹)
                <input type="number" min="0" step="0.05" required value={form.strike}
                  onChange={set('strike')} style={inp} placeholder="e.g. 22000" />
              </label>
              <label style={lbl}>Spot at Expiry (₹)
                <input type="number" min="0" step="0.05" required value={form.spot}
                  onChange={set('spot')} style={inp} placeholder="e.g. 22500" />
              </label>
            </>
          )}
        </div>
      ...
      {error && (
        <div style={{ ...notice, background: '#fee' }}>
          ❌ {error}
        </div>
      )}
      ...
      {result && (
        <div style={{ marginTop: '1.5rem' }}>
          {result.notes.map((n, i) => (
            <div key={i} style={{ ...notice, background: n.includes('⚠️') ? '#fff3cd' : '#d4edda' }}>
              {n}
            </div>
          ))}
          ...
          <p style={{ fontSize: '0.75rem', color: 'grey', marginTop: '0.5rem' }}>
            Hover any row for calculation basis. Amounts in INR, rounded to nearest paisa.
          </p>
        </div>
      )}

      <p style={{ color: '#ccc', fontSize: '0.75rem', marginTop: '2rem' }}>API: {API_URL}</p>
    </main>
  )
}
